...
export default function Postulaciones() {
  ...
  const buscarPorCedula = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/postulaciones/cedula/${encodeURIComponent(cedulaLimpia)}`,
      );
      ...
      Swal.fire(
        "Sin resultados",
        "No se encontro una postulacion con esa cedula",
        "info",
      );
    } finally {
      setLoading(false);
  ...
  const eliminarPostulacion = async (postulacion) => {
    ...
    try {
      ...
      setError("");
      await axios.delete(`${API_URL}/api/postulaciones/${postulacion.id}`);
      ...
      setError("Error eliminando la postulacion");
      Swal.fire("Error", "No se pudo eliminar la postulacion", "error");
    } finally {
      setDeletingId(null);
    }
  };
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {total} registro{total === 1 ? "" : "s"} disponible
              {total === 1 ? "" : "s"}.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="relative">
              ...
              <input
                ...
                className="h-11 w-full rounded-lg border border-slate-300 bg-white pl-10 pr-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 sm:w-72"
              />
            </div>

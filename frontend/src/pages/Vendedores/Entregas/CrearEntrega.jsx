...
const CrearEntrega = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 👇 SOLO AQUÍ agregamos el campo estado
      const payload = {
        ...formData,
        estado: "Pendiente", // o el valor que tu backend necesite
      };

      const res = await axios.post(`${API_URL}/entregas`, payload);
      ...
      navigate(`/entregas/${res.data.id}/detalles`, {
        state: {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Error al registrar la entrega",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg mt-10 border border-orange-100">
      <div className="mb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <input
          type="hidden"
          name="usuarioAgenciaId"
          value={formData.usuarioAgenciaId}
        />

        {/* Fecha */}
        <div>
          <label className="block mb-2 font-semibold text-gray-700">
            Fecha de registro para la Entrega *
          </label>
          <input
            type="date"
            name="fecha"
            ...
            className="w-full border border-orange-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-orange-500"
            required
          />
        </div>
        ...
        <div className="flex gap-4 pt-6">
          ...
          <button
            ...
            className={`flex-1 bg-orange-600 text-white font-semibold py-3 rounded-lg hover:bg-orange-700 ${
              loading || !formData.usuarioAgenciaId
          ...
          >
            {loading
              ? "Registrando..."
              : "Registrar Entrega y Agregar Productos"}
          </button>
        </div>
      </form>
    </div>

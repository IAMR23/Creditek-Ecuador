...
export default function VentaObsequioPage() {
  ...
  // Cargar obsequios de la venta
  const cargarDatos = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/venta-obsequios/obsequios/${ventaId}`
      );
      setLista(res.data);
    } catch (error) {
  ...
  const crearRegistro = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_URL}/venta-obsequios`, {
        ventaId,
        ...form,
      });
  ...
  const eliminar = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/venta-obsequios/obsequios/${id}`);
      cargarDatos();
    } catch (error) {
  ...
  return (
    <div className="p-6">
      ...
      <form
        ...
        className="bg-white p-4 rounded shadow mb-6 border border-green-500"
      >
